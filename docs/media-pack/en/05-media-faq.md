...
We are not AI researchers. We are operators who run six agents on a Mac Mini and got tired of the friction. Curiosity Cat is built by people with the problem, for people with the problem.

---

**How is this different from Guardrails AI or similar tools?**

Guardrails AI sits inside the agent's reasoning loop and validates outputs against a schema. It answers "is this response structured correctly." Curiosity Cat sits outside the agent and watches what the agent does in the world. It answers "is this action safe." They are complementary, not competitive. We run both.

The real gap in the market is the layer above the framework and below the human — the operator autonomy control plane. Nobody is building for the solo operator running agents on their own machine. We are.
...
Nothing. Open source, MIT licence. Install and use it, fork it, ship it in your own products, rebuild it from scratch if you want to. We do not sell the framework. We may in the future offer paid services on top of it — managed Danger Map curation, enterprise reporting, training — but the core is and stays free.
...
At Housecat: the agent asks permission before any action not in its standing orders. At Alley Cat: actions proceed unless they violate a standing order, and violations are logged and reported. At Tiger: actions proceed freely, everything is logged, and close-call reports are written after the fact.
...
Negligible for the pre-action check. Standing orders are JSON rules evaluated locally — microseconds per action. The logging has storage cost but does not slow the agent. Danger Map lookups are asynchronous and do not block execution.

---

**How does the Danger Map work?**

Close-call reports are submitted by operators after an event. The report describes what the agent tried to do, what nearly went wrong, and what the operator learned. The schema is open JSON. Reports are reviewed for quality (automated filters plus human curation) and published to a public map that anyone can search.
...
**Who funds Short+Sweet Agential?**

Short+Sweet International has funded its own work for 25 years. The festival business generates revenue. The Agential division is building the next layer of the business. We may take outside investment if the right partner appears. For now we are self-funded and moving at the pace that gives us.
...
No. This is for people who run AI agents and want to run more of them safely. The product assumes you already believe in AI, already have agents running, and already see the value. What it does is reduce the cost of supervising them. If you want fewer agents, Curiosity Cat is not for you. If you want more agents, responsibly, this is the layer that lets you.
...
They do good work and solve different problems. Paperclip operates at the organisational altitude — company, org chart, tickets, budgets. Twill operates in the autonomous coding space — close your laptop, come back to PRs. Curiosity Cat operates at the solo operator altitude — the person running agents on their own machine who wants to decide how much risk they will accept. We are layers in a stack, not competitors for the same customer.
